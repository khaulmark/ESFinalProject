...
void runCommand(int command, CSpider Spider, int Param);
			...
			if(!QueueCommand.IsEmpty() && QueueCommand.Pop(&Command, &Param) ){
				...
				switch(Command){
					...
					case CMD_SPPED:
						...
						break;
					case CMD_TURN_LEFT_DEGREE:
						printf("CMD_TURN_LEFT_DEGREE %d \n",Param);
						break;
					case CMD_TURN_RIGHT_DEGREE:
						printf("CMD_TURN_RIGHT_DEGREE %d \n",Param);
						break;
					...
					case CMD_JOYSTICK:
						...
						break;
					case CMD_ALL:
						//Override this command to play motions in reverse.
						printf("CMD_ALL\n");
						while(!commandHistory.empty()){
							runCommand(commandHistory.top(), Spider, Param);
							commandHistory.pop();
